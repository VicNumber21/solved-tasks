...
function createChain(context, chainRoads, roadsToUpdate) {
  ...
  for (let i = 0; i < subsets.length; ++i) {
    ...
    chain.cityCount += currentSubset.cityCount;
...
function createSimpleMix(leftSubset, rightSubset) {
  return  {
    ...
    roads: new Set(),
    cityCount: leftSubset.cityCount + rightSubset.cityCount,
...
function calculateRoadWeight(road) {
  ...
  return segmentWeight - leftSubset.cityCount - rightSubset.cityCount;
}

function calculateSegmentWeight(minCity, maxCity) {
  return  maxCity - minCity + 1;
}
...
class RoadMinBinHeap {
  ...
  _isLess (index1, index2) {
    ...
    return this._isInBounds(index1) && this._isInBounds(index2)
      && (road1.weight < road2.weight ||
          road1.weight === road2.weight && (road1.leftSubset.minCity < road2.leftSubset.minCity ||
          road1.leftSubset.minCity === road2.leftSubset.minCity && road1.rightSubset.minCity < road2.rightSubset.minCity));
  }
